...
def train_fold(train_folds, val_fold, patience=5):
    # Загружае   CSV
    train_dfs = [pd.read_csv(f"/kaggle/input/datasets/andreikarabin/data3better/aspirantura/PROF/npy_article_fold/train_article_fold_{f}.csv") for f in train_folds]
    df_train = pd.concat(train_dfs).reset_index(drop=True)
    df_val = pd.read_csv(f"/kaggle/input/datasets/andreikarabin/data3better/aspirantura/PROF/npy_article_fold/train_article_fold_{val_fold}.csv")

    BASE_PATH = "/kaggle/input/datasets/andreikarabin/data3better/aspirantura/PROF/npy_article_fold"
